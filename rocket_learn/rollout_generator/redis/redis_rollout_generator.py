...
from trueskill import SIGMA, Rating, rate
...
from rocket_learn.rollout_generator.redis.utils import (
    ...
    decode_buffers,
    get_pretrained_rating,
    get_pretrained_ratings,
    get_rating,
    get_ratings,
)
...
class RedisRolloutGenerator(BaseRolloutGenerator):
    ...
    def __init__(
        ...
        if pretrained_agents is not None:
            add_pretrained_ratings(self.redis, pretrained_agents, gamemodes=gamemodes)
        ...
        if clear:
            ...
            self.redis.decr(
                VERSION_LATEST, max_age + 1
            )  # In case of reload from old version, don't let current seep in
    ...
    def _process_rollout(
        ...
        (
            ...
            has_states,
            has_rewards,
        ) = _unserialize(rollout_bytes)

        v_check = [v for v in versions if isinstance(v, int) or v.startswith("-")]

        if any(
            version < 0 and abs(version - latest_version) > max_age
            for version in v_check
        ):
            return

        if any(version < 0 for version in v_check):
            buffers, states = decode_buffers(
                rollout_data,
                versions,
                has_obs,
                has_states,
    ...
    def _update_ratings(self, name, versions, buffers, latest_version, result):
        ...
        has_buffer_versions = [
            ...
            or (
                v != "human"
                and (
                    "-".join(v.split("-")[:-1]) not in self.pretrained_agents_keys
                    and v not in self.pretrained_agents_keys
                )
            )
        ]
        ...
        rated_versions = [v for v in versions if isinstance(v, str) and v != "human"]
        for version in rated_versions:
            short_name = "-".join(version.split("-")[:-1])
            if (
                version in self.pretrained_agents_keys
                or short_name in self.pretrained_agents_keys
            ):
                rating = get_pretrained_rating(gamemode, version, self.redis)
            else:
        ...
        if len(ratings) == len(versions) and len(buffers) == 0:
            ...
            for version, ratings in ratings_versions.items():
                ...
                avg_rating = Rating(
                    sum(r.mu for r in ratings) / len(ratings),
                    max(
                        sum(r.sigma**2 for r in ratings) ** 0.5 / len(ratings),
                        self.min_sigma,
                    ),
                )
                short_name = "-".join(version.split("-")[:-1])
            ...
            if mapping_past:
                self.redis.hset(QUALITIES.format(gamemode), mapping=mapping_past)
            if mapping_pretrained:
                self.redis.hset(
                    PRETRAINED_QUALITIES.format(gamemode), mapping=mapping_pretrained
                )
        ...
        if len(relevant_buffers) > 0:
            self.redis.hincrby(
                EXPERIENCE_PER_MODE,
                gamemode,
                len(relevant_buffers) * relevant_buffers[0].size(),
            )
    ...
    def generate_rollouts(self) -> Iterator[ExperienceBuffer]:
        while True:
            ...
            res = self._process_rollout(
                ...
                latest_version,
                self.obs_build_func,
                self.rew_func_factory,
                self.act_parse_factory,
                self.max_age,
            )
            if res is not None:
        ...
        for gamemode in gamemodes:
            ...
            for mode in "stochastic", "deterministic":
                if gamemode != "mean":
                    ...
                    mus = [
                        mean[0] / len(self.gamemodes) for mean in means_mode.values()
                    ]
                    sigmas = [
                        (mean[1] / len(self.gamemodes)) ** 0.5
    ...
    def _add_opponent(self, agent):
        ...
        for gamemode in self.gamemodes:
            ...
            for mode in "stochastic", "deterministic":
                ...
                self.redis.hset(
                    QUALITIES.format(gamemode),
                    f"{key}-{mode}",
                    _serialize(tuple(quality)),
                )
    ...
    def update_parameters(self, new_params_actor, new_params_critic, iteration):
        ...
        if self.pretrained_agents_keys:
            ...
            for gamemode in self.gamemodes:
                ...
                if len(normal_ratings_dict) > 0:
                    non_pretrained_min_rating = min([r.mu for r in normal_ratings_dict])
                else:
            ...
            for gamemode in self.gamemodes:
                self.logger.log(
                    {
                        "pretrained/qualities-"
                        + gamemode: wandb.Table(
                            columns=["name", "rating"],
                            data=pretrained_qualities[gamemode],
                        )
                    },
                    commit=False,
                )
        ...
        tot_contributors = Counter(
            {name: int(count) for name, count in tot_contributors.items()}
        )
        ...
        if n_updates > 0:
            self.logger.log(
                {f"stat/{name}": value for name, value in self._get_stats().items()},
                commit=False,
            )
        self._reset_stats()
